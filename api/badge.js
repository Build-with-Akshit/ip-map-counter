/**
 * GET /api/badge?username=XXX
 *
 * Renders a glowing, GitHub-dark-themed Call-To-Action SVG badge button
 * for embedding above the main analytics dashboard in GitHub READMEs.
 */
export default function handler(req, res) {
  const username = req.query.username || "Build-with-Akshit";

  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="480" height="42" viewBox="0 0 480 42">
    <defs>
      <linearGradient id="badgeBg" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop offset="0%" stop-color="#161b22"/>
        <stop offset="100%" stop-color="#0d1117"/>
      </linearGradient>

      <linearGradient id="borderGlow" x1="0%" y1="0%" x2="100%" y2="0%">
        <stop offset="0%" stop-color="#238636"/>
        <stop offset="50%" stop-color="#39d353"/>
        <stop offset="100%" stop-color="#2ea043"/>
      </linearGradient>

      <filter id="neonPulse" x="-50%" y="-50%" width="200%" height="200%">
        <feGaussianBlur stdDeviation="3" result="blur"/>
        <feComposite in="SourceGraphic" in2="blur" operator="over"/>
      </filter>
    </defs>

    <style>
      @import url('https://fonts.googleapis.com/css2?family=Inter:wght@500;600;700;800&amp;display=swap');
      .badge-text {
        font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
        font-size: 13px;
        font-weight: 600;
        fill: #c9d1d9;
      }
      .highlight {
        fill: #39d353;
        font-weight: 700;
        text-decoration: underline;
      }
    </style>

    <!-- Outer Card Container -->
    <rect x="1" y="1" width="478" height="40" rx="9" fill="url(#badgeBg)" stroke="url(#borderGlow)" stroke-width="1.2"/>

    <!-- Left Pulsing Green Icon Badge -->
    <g transform="translate(14,13)">
      <circle cx="8" cy="8" r="7" fill="#0e4429" stroke="#238636" stroke-width="1"/>
      <circle cx="8" cy="8" r="3.5" fill="#39d353" filter="url(#neonPulse)"/>
    </g>

    <!-- Text content -->
    <text x="44" y="26" class="badge-text">
      👉 Please <tspan class="highlight">CLICK HERE</tspan> to contribute a view in my profile 🌐
    </text>
  </svg>`;

  res.setHeader("Content-Type", "image/svg+xml");
  res.setHeader("Cache-Control", "no-store, no-cache, must-revalidate, max-age=0");
  return res.status(200).send(svg);
}
